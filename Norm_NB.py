...
# word_count = spark.udf.register("word_count",lambda row : sum(len(char) for char in row),"integer")
...
# tf = HashingTF(inputCol="filtered", outputCol="hash_tf")
# idf = IDF(inputCol="hash_tf", outputCol="tfidf")

# normalizer = Normalizer(inputCol="tfidf", outputCol="norm_tfidf")


# clean_data = VectorAssembler(inputCols = ['norm_tfidf', 'funny'], outputCol = 'features')

# # pipeline = Pipeline(stages=[SQLTransformer(statement = "SELECT *, word_count(text) length FROM __THIS__"),
# #                             tokenizer, stopwordsRemover, tf, idf, normalizer, clean_data])

# pipeline = Pipeline(stages=[tokenizer, stopwordsRemover, tf, idf, normalizer, clean_data])


# pipelineFit = pipeline.fit(df)

# # pipelineFit.save(sys.argv[1] + '/Log_NBnorm_pipeline')

# raw_dataset = pipelineFit.transform(df)
